...
if (isset($_POST['employee'])) {
	...
	if ($query->num_rows > 0) {
		...
		if ($status == 'in' || $status == 'ijin' || $status == 'sakit') {
			...
			if ($query->num_rows > 0) {
				...
				//
				$sql = "INSERT INTO attendance (employee_id, date, time_in, status, status_in) VALUES ('$id', '$date_now', NOW(), '$logstatus','$status')";
				if ($conn->query($sql)) {
			...
			if ($query->num_rows < 1) {
				...
				if ($row['time_out'] != '00:00:00') {
					...
					$waktu_sekarang = strtotime(date("H:i"));
					// Mengubah waktu 17:00 menjadi format yang sama (H:i)
					$waktu_tujuan = strtotime($schedule['time_out']);
					// Memeriksa apakah waktu saat ini lebih besar dari atau sama dengan waktu 17:00
					if ($waktu_sekarang >= $waktu_tujuan) {
						...
						$output['error'] = true;
						$output['message'] = "Anda tidak bisa absen keluar, karena belum jam {$schedule['time_out']}";
					}
				}
			}
		}
